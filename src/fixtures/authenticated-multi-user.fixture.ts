 ...
 */
export const authenticatedMultiUserTest = base.extend<AuthenticatedMultiUserContext>({
  /**
  ...
  user1Page: async ({ user1Context }, use) => {
    ...
    if (isLoginButtonVisible) {
      ...
      if (config.user1.email && config.user1.password) {
        ...
        console.warn('⚠️  User 1 credentials not configured. Cannot authenticate.');
      }
    } else {
  ...
  user2Page: async ({ user2Context }, use) => {
    ...
    if (isLoginButtonVisible) {
      ...
      if (config.user2.email && config.user2.password) {
        ...
        console.warn('⚠️  User 2 credentials not configured. Cannot authenticate.');
      }
    } else {
      // User already logged in
  ...
  user3Page: async ({ user3Context }, use) => {
    ...
    if (isLoginButtonVisible) {
      ...
      if (config.user3.email && config.user3.password) {
        ...
        console.warn('⚠️  User 3 credentials not configured. Cannot authenticate.');
      }
    } else {
      // User already logged in
